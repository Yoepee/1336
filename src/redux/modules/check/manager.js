...
// async를 통한 비동기로 데이터를 받아오는 과정 (미들웨어 공부하시면 좋을듯)
  export const __chkManager = createAsyncThunk(
    "/api/roleCheck",
    async (payload, thunkAPI) => {
        try {
            const data =  await axios.get("http://3.34.5.30:8080/api/roleCheck", {
                headers: {
...
export const manager = createSlice({
    ...
    extraReducers: {
        [__chkManager.pending]: (state) => {
          state.isLoading = true; // 네트워크 요청이 시작되면 로딩상태를 true로 변경합니다.
        },
        [__chkManager.fulfilled]: (state, action) => {
          state.isLoading = false; // 네트워크 요청이 끝났으니, false로 변경합니다.
          state.data = action.payload; // Store에 있는 todos에 서버에서 가져온 todos를 넣습니다.
        },
        [__chkManager.rejected]: (state, action) => {
